...
void imod::ScaleDown(cv::Mat & resultImage, const cv::Mat & originalImage, int scaleFactor) {
...
cv::Mat imod::ScaleDown(const cv::Mat & originalImage, int scaleFactor) {
	cv::Mat ret; ScaleDown(ret, originalImage, scaleFactor);
	return ret;
}


void imod::ScaleUp(cv::Mat & resultImage, const cv::Mat & originalImage, int scaleFactor) {
	cv::Scalar scalar((originalImage.channels() == 3) ? 0, 0, 0 : (originalImage.channels() == 2) ? 0, 0 : 0);
	resultImage = cv::Mat(originalImage.rows * scaleFactor, originalImage.cols * scaleFactor, originalImage.type(), scalar);
...
cv::Mat imod::ScaleUp(const cv::Mat & originalImage, int scaleFactor) {
	cv::Mat ret; ScaleUp(ret, originalImage, scaleFactor);
	return ret;
}
